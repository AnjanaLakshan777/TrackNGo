...
import {
  ...
  faComments, faFloppyDisk, faPen, faPlus, faSearch, faSpinner, faStar, faTrash, faUser, faXmark,
...
// Local date, not UTC. toISOString() converts to UTC first, so in Sri Lanka
// (UTC+5:30) it returns yesterday until 05:30 - and validateAdminDriverRequest
// compares against the local date, so the two disagreed every morning.
...
function defaultExpiry() { const date = new Date(); date.setFullYear(date.getFullYear() + 1); return isoLocal(date) }
...
function Driver() {
  ...
  function openDriverChat(driver: Driver) { const path = getAdminChatPath(driver.id, 'Driver'); if (path) navigate(path) }
