 ...
 *
 * Shared verbatim with the passenger app (mobile-app/TrackNgo-Mobile/components/
 * ProfileAvatarPlaceholder.tsx), which in turn redraws the illustration the
 * admin web app uses, so a person without a photo looks the same everywhere in
 * the product. The two apps are separate Expo packages with no shared
 * workspace, so this file is duplicated rather than imported; keep the copies
 * in sync.
 *
 * The artwork paints its own circular background, so it does not need a
 * coloured container behind it.
 */
